 ...
 * 负责将用户调用的本地方法（如
 * stub.Login()）转换为网络请求发送出去，并接收服务端的响应。
 *
 * 继承自 google::protobuf::RpcChannel，这是 Protobuf 为 RPC
 * 定义的标准抽象接口。 任何想要使用 Protobuf 生成的 Stub 类的 RPC
 * 框架，都必须实现这个接口中的 CallMethod 方法。
 *
 * 模块作用
 * 统一通信入口：它是所有 RPC 调用的底层入口。无论你调用 Login 还是
 * Register，最终都会走到 CallMethod。
 *
